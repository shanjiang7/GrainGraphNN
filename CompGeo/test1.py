...
class graph:
    ...
    def snap(self, fname='snap'):
        ...
        for region_id, coors in self.region_coors.items():
            ...
            # Plot the region id at the centroid.
        ...
        if hasattr(self, 'added_edge'):
            for edge in self.added_edge:
                ...
                ax.plot([pt1[0], pt2[0]], [pt1[1], pt2[1]], color='green', linestyle='--', linewidth=2)
    ...
    def add_polygon(self, center=(0.5, 0.5), radius=0.1, perturb_ratio=0.05):
        ...
        covered = self.find_vertices_covered_by_polygon(polygon_points)
        if covered:
        ...
        for region in set(self.nucle_fall_region):
            ...
            for v_id in self.nucle_map[region]: 
                ...
                if current_edge in edge_map and edge_map[current_edge][0]>-1 and edge_map[current_edge][1]>-1:
                    self.vertex2joint[v_id].discard(region)
                    self.vertex2joint[v_id].update(edge_map[current_edge][:2])

                    inner = current_edge[1] if current_edge[0] == v_id else current_edge[0]
                ...
                if edge_map[current_edge][0] == -1:
                    ...
                    all(left_candidate not in edge for edge in self.added_edge)):
                        ...
                        # print("check",self.vertex2joint[left_candidate])
                ...
                if edge_map[current_edge][1] == -1:
                    ...
                    region_count += 1
                    # print("right")
                    print("2", edge_map[current_edge])

                    right_candidate=sorted_candidates[(idx+1)%n]
                    print("right", right_candidate)
    ...
    def find_vertices_covered_by_polygon(self, coords, tol=1e-16):
        """
    ...
    def connect(self):
        ...
        for region, new_vertices in self.nucle_map.items():
            ...
            new_vertices=sort_indices_clockwise(self, new_vertices)

            for idx in new_vertices:
                ...
                if chose_idx==None:
                    chose_idx=self.projection(idx, sorted_vertices)
                else:
                    sorted_vertices.remove(chose_idx)
                print("choose" , chose_idx)
                self.added_edge.append((idx, chose_idx))
...
if __name__ == '__main__':
    g1 = graph(lxd = 40)
    g1.update(init=True)
    # g1.add_polygon(center=(0.40, 0.55), radius=0.08, perturb_ratio=0.1)
    # g1.add_polygon(center=(0.2, 0.2), radius=0.06, perturb_ratio=0.2)
    # g1.add_polygon(center=(0.3, 0.3), radius=0.06, perturb_ratio=0.2)
    # g1.add_polygon(center=(0.3, 0.65), radius=0.06, perturb_ratio=0.2)
    # g1.add_polygon(center=(0.4, 0.5), radius=0.06, perturb_ratio=0.2)
    # g1.add_polygon(center=(0.5, 0.73), radius=0.06, perturb_ratio=0.2)
    # g1.add_polygon(center=(0.65, 0.3), radius=0.06, perturb_ratio=0.2)
    # g1.add_polygon(center=(0.73, 0.78), radius=0.04, perturb_ratio=0.2)
    g1.add_polygon(center=(0.34, 0.475), radius=0.06, perturb_ratio=0.1)
    # g1.remove_vertex()
    # g1.connect()
    g1.update()
    g1.snap()
